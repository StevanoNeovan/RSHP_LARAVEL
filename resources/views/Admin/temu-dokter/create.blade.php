...
@section('page-title', 'Buat Reservasi Temu Dokter')
    ...
    <div class="form-card">
        <div class="form-card-header">
            <h3>
                ...
                Form Reservasi Temu Dokter
            </h3>
            <p>Buat reservasi baru untuk pemeriksaan hewan</p>
        </div>

        <!-- Nomor Antrian Preview -->
        <div class="number-preview">
            <h4>Nomor Antrian Hari Ini</h4>
            <div class="number">{{ str_pad($nextNumber, 3, '0', STR_PAD_LEFT) }}</div>
        </div>
        ...
        <form action="{{ route('admin.temu-dokter.store') }}" method="POST">
            ...
            <div class="form-group">
                ...
                <div class="radio-group">
                    <label class="radio-card" onclick="this.classList.add('selected'); document.querySelectorAll('.radio-card').forEach(c => c !== this && c.classList.remove('selected'))">
                        <input type="radio" name="status" value="W" {{ old('status', 'W') == 'W' ? 'checked' : '' }} required>
                    ...
                    <label class="radio-card" onclick="this.classList.add('selected'); document.querySelectorAll('.radio-card').forEach(c => c !== this && c.classList.remove('selected'))">
                        <input type="radio" name="status" value="P" {{ old('status') == 'P' ? 'checked' : '' }} required>
                        <div class="radio-card-content">
                    ...
                    <label class="radio-card" onclick="this.classList.add('selected'); document.querySelectorAll('.radio-card').forEach(c => c !== this && c.classList.remove('selected'))">
                        <input type="radio" name="status" value="S" {{ old('status') == 'S' ? 'checked' : '' }} required>
                        <div class="radio-card-content">
                    ...
                    <label class="radio-card" onclick="this.classList.add('selected'); document.querySelectorAll('.radio-card').forEach(c => c !== this && c.classList.remove('selected'))">
                        <input type="radio" name="status" value="B" {{ old('status') == 'B' ? 'checked' : '' }} required>
                        <div class="radio-card-content">
...
<script>
    ...
    // Set initial selected radio card
