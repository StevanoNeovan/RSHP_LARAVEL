...
<div class="card header-section">
    ...
    <a href="{{ route('admin.kategori.create') }}">
        <button class="btn btn-primary">
            <i class="fas fa-plus"></i> Tambah Kategori Tindakan
        </button>
    </a>
</div>
...
<div class="filter-tabs">
    ...
    <a href="{{ route('admin.kategori.index', ['trashed' => 'only']) }}"
       class="btn {{ request('trashed') == 'only' ? 'btn-danger' : 'btn-secondary' }}">
        <i class="fas fa-trash"></i> Terhapus
    </a>

    <a href="{{ route('admin.kategori.index', ['trashed' => 'with']) }}"
...
<div class="alert alert-warning">
    ...
    <a href="{{ route('admin.kategori.index') }}" class="fw-bold">Lihat data aktif</a>
</div>
@endif
    ...
    <table>
        <thead>
            <tr>
                ...
                <th class="nama-kategori">Nama Kategori</th>
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $index + 1 }}</td>
                <td class="nama-kategori">{{ $kat->nama_kategori }}</td>
                <td>
                    @if($kat->trashed())
                ...
                <td>
                    <div class="action-group">
                        @if($kat->trashed())
                            <form action="{{ route('admin.kategori.restore', $kat->idkategori) }}"
                                  method="POST">
                                @csrf
                                @method('PATCH')
                            ...
                            <form action="{{ route('admin.kategori.force-delete', $kat->idkategori) }}"
                                  method="POST"
